...
import { promoteIfActNow } from "../lib/promote-idea";
    ...
    for (const inventor of activeInventors) {
      try {
        ...
        for (const idea of ideas) {
          ...
          if (insertErr) {
            console.error(`Failed to insert inventor idea "${idea.title}":`, insertErr);
            continue;
          }

          // Only self-claimed act_now ideas get promoted into real panel review --
          // promoting everything would multiply Opus panel calls unnecessarily.
          if (idea.priority_flag === "act_now" && insertedIdea) {
            const note = `${idea.concept || ""}\n\nWhy now (self-assessed by ${inventor.name}): ${idea.priority_reasoning || "(not given)"}\n\nGenerated by: Inventor Pool -- ${inventor.name}`;
            await promoteIfActNow(supabase, "inventor_ideas", insertedIdea.id, null, idea.title, note);
          }
        }
