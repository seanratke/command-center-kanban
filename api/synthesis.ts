...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    for (const idea of result.ideas || []) {
      const ideaType = idea.idea_type === 'overlap' ? 'overlap' : 'combination';
      ...
      if (insertErr) {
        console.error(`Failed to insert synthesis idea "${idea.title}":`, insertErr);
        continue;
      }
      ...
      if (idea.priority_flag === "act_now" && insertedIdea) {
        const note = `${idea.concept || ""}\n\nWhy now (self-assessed): ${idea.priority_reasoning || "(not given)"}\n\nGenerated by: Synthesis Engine (${ideaType})`;
        await promoteIfActNow(supabase, "synthesis_ideas", insertedIdea.id, null, idea.title, note);
      }
    }
