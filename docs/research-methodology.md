...
## Seed-idea watchlist -- found broken, fixed 2026-09-15

The watchlist backend (`api/run-opportunity-engine.ts` fetching active `seed_ideas`, appending them to the prompt, and inserting into `seed_idea_flags` / updating `last_flagged_date` for any `watchlist_flags` match) has existed since commit `3da259b` (2026-09-13). It never fired once -- `seed_idea_flags` had zero rows across 3+ real run-days since the one seed idea was created. Root cause: the system prompt (`lib/opportunity-engine-prompt.md`) never mentioned the watchlist at all, and its OUTPUT JSON template never included a `watchlist_flags` field, so Claude was never told the mechanism existed or asked to use it -- the backend code was correct and simply never received input. Fixed by adding a WATCHLIST CHECK step to the prompt (actually compare today's research against each active watchlist idea, flag only on a genuine match, not a keyword-level resemblance) and adding `watchlist_flags` to the OUTPUT JSON schema.
