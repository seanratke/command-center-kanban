...
**Known gap, not yet fixed:** the `research_grounding` field only exists in the schema for Human Needs board items. The `main`, `far_out`, and `canada_bc` boards now also search for real, but their output schema has no field to capture what was found or cite it -- so real search happens for all four boards, but only Human Needs items currently surface their sources in the data. Worth deciding separately whether to add an equivalent citation field to the other three boards.
